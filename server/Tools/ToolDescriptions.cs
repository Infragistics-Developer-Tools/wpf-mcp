...
// Verbatim copies of TOOL_DESCRIPTIONS in src/tools/constants.ts — keep both in sync.
public static class ToolDescriptions
{
    public const string ListWpfComponents = """
        List Infragistics WPF controls with their canonical XAML namespace URI, recommended xmlns prefix, required NuGet package, and description.
    ...
    public const string GetWpfApiReference = """
        Get full API documentation for an Infragistics WPF type: xmlns declaration, NuGet package, type summary, and all members (properties, methods, events) with descriptions.
    ...
    public const string GetWpfProjectScaffold = """
        Generate dotnet CLI setup commands and XAML xmlns declarations for a new Infragistics WPF project.
    ...
    public const string SearchWpfApi = """
        Search Infragistics WPF API entries by keyword across type names, summaries, and member names. Multi-word queries use ranked OR matching — a type matching more words ranks higher, but matching just one word still returns it, so a natural-language phrase won't come back empty.
    ...
    public const string SearchWpfDocs = """
    ...
    public const string GetWpfDoc = """
        Retrieve the full text and extracted XAML code examples for one WPF documentation topic — any topic type: getting started, layouts/nesting, styling/theming, data binding, editing/validation, filtering/sorting/grouping/summaries, exporting, performance, commands, persistence, known issues, breaking changes, etc. Not limited to structural/nesting topics.
    ...
    public const string SetupWpfTheme = """
        List available Infragistics WPF named themes and the raw XAML style/resource-dictionary files backing them, sourced directly from the official Infragistics/wpf-resources repository. Covers BOTH theming mechanisms used across the product — they are NOT interchangeable, always check which one applies before writing code:
    ...
    public const string GetWpfThemeResource = """
        Retrieve the full raw XAML content of one theme/style resource-dictionary file from the official Infragistics/wpf-resources repository — real Style/ControlTemplate/brush definitions, not summarized.
    ...
    public const string GetWpfThemePalette = """
        Return the centralized color/brush PALETTE of one newer-family (ThemeManager) Infragistics WPF theme — the exact `<Color>` and `<SolidColorBrush>` resource keys, current values, and semantic groupings parsed from that theme's official `<Theme>.Theme.Colors.xaml`. Use this for the specific task of RE-COLORING an existing theme (tuning the palette) WITHOUT creating a brand-new theme.
